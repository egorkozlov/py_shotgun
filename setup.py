...
class ModelSetup(object):
    def __init__(self,nogrid=False,divorce_costs_k='Default',divorce_costs_nk='Default',**kwargs): 
        ...
        if p['high education']:            
            ...
            p['sig_zm']    = 0.09153921
            p['sig_zm_0']  = 0.3336769
            
            
            p['sig_zf']    = 0.0837479
            p['sig_zf_0']  = 0.31071877
        ...
        # no college
            ...
            p['f_wage_trend'] = np.array(
                                [-.3715448 + 
                                 .0258538*(min(t,30) - 5)
                                 -.0012931*((min(t,30)-5)**2)
                                 +.0000273*((min(t,30)-5)**3)
                                             for t in range(T)]
                                        )
            
            p['m_wage_trend'] = np.array(
                                        [-.2587716 + 0.0348409*(min(t,30)-5)  
                                             -.0013866*((min(t,30)-5)**2) 
                                             + 0.000024*((min(t,30)-5)**3)
                                             for t in range(T)]
                                        )
